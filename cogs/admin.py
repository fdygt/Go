...
class AdminCog(commands.Cog, name="Admin"):
    ...
    async def db(self, ctx):
        ...
        try:
            ...
            dashboard_url = f"http://localhost:8080/admin/auth?token={token}"
    
            # Kirim HANYA melalui DM
            try:
                await ctx.author.send(f"🔒 Your secure dashboard link (valid for 24 hours):\n{dashboard_url}")
                return  # Keluar dari function jika berhasil
            except discord.Forbidden:
                # Jika gagal kirim DM, beri tahu user untuk mengaktifkan DM
                await ctx.send("❌ I couldn't send you the dashboard link. Please enable DMs from server members.")
                return
    
        except Exception as e:
            logger.error(f"Dashboard link error: {e}")
            await ctx.send("❌ Error generating dashboard link. Please try again.")
